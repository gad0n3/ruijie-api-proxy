...
function createDemoRoutes() {
  ...
  router.post('/packages/create', (req, res) => {
    ...
    const row = {
      id,
      userGroupName: payload.userGroupName || payload.name || `Demo Package ${id}`,
      authProfileId,
  ...
  router.delete('/packages/:uuid', (req, res) => {
    ...
    demoState.packages = demoState.packages.filter((item) => {
      ...
      if (authProfileId && String(item.authProfileId) === String(authProfileId)) {
        return false;
      }
      return true;
    });

    demoState.vouchers = demoState.vouchers.filter((item) => String(item.profileId) !== String(authProfileId));
  ...
  router.get('/vouchers', (req, res) => {
    ...
    const status = typeof req.query.status === 'undefined' ? null : String(req.query.status);

    const list = demoState.vouchers.filter((item) => {
  ...
  router.get('/vouchers/status', (req, res) => {
    ...
    const list = demoState.vouchers.filter((item) => String(item.groupId) === groupId);
  ...
  router.post('/vouchers/generate', (req, res) => {
    ...
    const userGroupId = String(payload.userGroupId || demoState.packages[0]?.id || '500001');
    const profileId = String(payload.profile || payload.authProfileId || demoState.packages[0]?.authProfileId || 'demo-auth-1001');
  ...
  router.delete('/vouchers/expired', (req, res) => {
    ...
    const ids = new Set(payload.map((item) => String(item.uuid || '')));

    const before = demoState.vouchers.length;
    demoState.vouchers = demoState.vouchers.filter((item) => !ids.has(String(item.uuid)));
    ...
    res.json({
      ...
      msg: 'Success.',
      deletedCount,
      batchCount: payload.length > 0 ? 1 : 0
    });
  });
  ...
  router.get('/clients/unauth', (req, res) => {
    res.json({
      list: demoState.clients.unauth
    });
  });

  router.get('/clients/suspected', (req, res) => {
    res.json({
      list: demoState.clients.suspected
    });
  });
